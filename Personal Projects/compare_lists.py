...
TOP_N   = 200   # We'll compare top 250 from each list
...
def load_bestball_ranks(filepath, top_n=250, delimiter=",", encoding="utf-8-sig"):
    """
    Reads the new BestBall CSV with columns: 'Rank', 'Player', etc.
    Sort ascending by Rank, keep top_n => rank=1..N.
    ...
    with open(filepath, newline="", encoding=encoding) as f:
        ...
        for row in reader:
            total_rows += 1
            name_raw = row.get("Player") or row.get("﻿Player") or ""
            name = name_raw.strip()
            ...
            players.append((name, rank_val))

    # sort ascending by Rank
    players.sort(key=lambda x: x[1])

    bestball_dict = {}
    for i, (full_name, rank_val) in enumerate(players[:top_n], start=1):
        norm = normalize_name(full_name)
        bestball_dict[norm] = i

    print(f"[BestBall] {filepath}: read {total_rows} rows, used {len(bestball_dict)} (lowest {top_n} Ranks).")
